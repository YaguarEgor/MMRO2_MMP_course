...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        for sent in parallel_corpus:
            s, t = sent.source_tokens, sent.target_tokens
            probs = self.translation_probs[s[:,None], t].astype(np.float32)
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        elbo = 0.0
        eps = 1e-12
        for sent, q in zip(parallel_corpus, posteriors):
            s = sent.source_tokens
            t = sent.target_tokens
            probs = self.translation_probs[s[:, None], t]
            elbo += np.sum(q * np.log(probs + eps) - q*np.log(q+eps) - q * np.log(len(s)))
        return elbo
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for sent, q in zip(parallel_corpus, posteriors):
            s = sent.source_tokens
            t = sent.target_tokens
            np.add.at(self.translation_probs, (s[:, None], t), q)

        row_sums = self.translation_probs.sum(axis=1, keepdims=True)
        row_sums[row_sums==0] = 1e-10
        self.translation_probs /= row_sums
